# Actividad-Elementos-Programa-Python 

Voy a hacer una documentacion de el archivo lavadero.py
...
Este bloque crea la clase Lavadero y define números fijos para cada fase del lavado
...
Cuando se crea un nuevo lavadero, este método pone todo a cero: sin dinero, inactivo, libre y sin extras seleccionados.
...
```python
Permiten leer el estado del lavadero desde fuera (por ejemplo lavadero.fase) pero nadie puede cambiarlo directamente. Es la forma segura de consultar los datos privados
```
...
Deja el lavadero completamente limpio y libre cuando el coche ya ha terminado todo el proceso.
...
Aquí llega un nuevo cliente. Se comprueba que el túnel esté libre y que no pida encerado sin secado a mano. Si todo está bien, se marcan las opciones y el lavadero pasa a estar ocupado
...
Calcula cuánto cuesta este lavado según los extras y lo suma al dinero total del día.
...
Es el corazón del programa. Cada vez que lo llamas, el lavadero pasa al siguiente paso: cobrar → prelavado (si toca) → agua → jabón → rodillos → secado → encerado (si toca) → terminar.
...
Convierte el número de fase en un texto bonito para que se entienda al verlo por pantalla
...
Muestra absolutamente todo lo que está pasando en el lavadero en un momento dado. Ideal para ver cómo evoluciona.
...
Solo se usa en pruebas automáticas. Ejecuta un lavado completo de una vez y devuelve la lista exacta de fases por las que pasó (muy útil para comprobar que todo funciona bien).
